...
def main():
    ...
    if resp.status_code == 200:
        ...
        vader= resp.json()
        # pprint(vader)
        print(f'{vader["name"]} was born in the year {vader["birth_year"]}.  His eyes are now {vader["eye_color"]} and his hair color is {vader.get("hair_color")}')
    
        resp = requests.get("https://swapi.dev/api/films/1/")
        movie_info = resp.json()
        print(f'He first appeared in the movie "{movie_info.get("title")}"')
